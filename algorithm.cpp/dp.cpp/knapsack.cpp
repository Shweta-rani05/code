...
// timecomplexity = o(nlog n) {o(n)+o(nlogn) + o(n)}
...
int fractionalKnapsack(vector<int> val,vector<int> wt, int W){
    int n = val.size();
    vector<pair<double,int>> ratio(n,make_pair(0.0,0)); //pair(ration,index) -> (double,int)
    for(int i=0 ;i<n; i++){
        double r = val[i]/(double)wt[i]; //typecasting 
